 ...
 */
    ...
    public static int[] union (int ar1[], int ar2[]) {
        ...
        int[] copy = Arrays.copyOf(ar1, ar1.length);
        ArrayInt.sort(copy);
        for (int i = 0; i < ar2.length; i++) {
            if (ArrayInt.binarySearch(copy, ar2[i]) < 0) {
                res[unique++] = ar2[i];
            }
        }
        return Arrays.copyOf(res, unique);
    }
    /**
     * Assumption: no repeated numbers in each array, but
     * numbers in first array may be repeated in the second
    ...
    public static int[] intersection (int ar1[], int ar2[]) {
        ...
        int index = 0;
        int[] copy = Arrays.copyOf(ar2, ar2.length);
        ArrayInt.sort(copy);
        for (int i = 0; i < ar1.length; i++) {
            if (ArrayInt.binarySearch(copy, ar1[i]) >= 0) {
                res[index++] = ar1[i];
            }
        }
